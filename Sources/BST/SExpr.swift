...
//   tree  := "E" | "(E)" | "(T" <tree> <int> <int> <tree> ")"
//   tuple := "(" <arg> ... ")"
// e.g. the witness `((T E 2 1 E) 0 2 0)` is a 4-tuple whose first element is a
// tree. This is the same wire format the language-agnostic runners consume, and
// it's what `etna.toml` witnesses are written in.
...
public func decodeTree(_ e: SExpr) throws -> Tree {
    ...
    case let .list(items):
        ...
        guard items.count == 5, items[0] == .atom("T") else {
            throw DecodeError.malformed("not a tree node: \(e)")
        }
        return .T(try decodeTree(items[1]), try decodeInt(items[2]), try decodeInt(items[3]), try decodeTree(items[4]))
    default:
...
/// Returns `nil` when the precondition discards the input.
public func evaluate(property: String, args: [SExpr]) throws -> Bool? {
    ...
    case "InsertUnion":
        return prop_insert_union(try tree(0), try tree(1), try int(2), try int(3))
    case "DeleteInsert":
    ...
    case "DeleteUnion":
        return prop_delete_union(try tree(0), try tree(1), try int(2))
    case "UnionDeleteInsert":
        return prop_union_delete_insert(try tree(0), try tree(1), try int(2), try int(3))
    case "UnionUnionIdem", "UnionUnionIdempotent":
        return prop_union_union_idempotent(try tree(0))
    case "UnionUnionAssoc":
        return prop_union_union_assoc(try tree(0), try tree(1), try tree(2))
    default:
