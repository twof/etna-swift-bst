...
//   bst-sampler <property> <count>
// Emits a JSON array of {"time","value"} — generation time and the ETNA wire
// serialization of each generated input. Open-loop (no coverage feedback).

let args = CommandLine.arguments
guard args.count >= 3, let count = Int(args[2]) else {
    FileHandle.standardError.write(Data("usage: bst-sampler <property> <count>\n".utf8))
    exit(2)
}
let property = args[1]
